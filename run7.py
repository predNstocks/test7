...
def calculate_continuous_alloc_v2(z_score, yield_spread, ratio):
    # 1. VALUATION COMPASS (Blend of Z-Score and Ratio)
    # We want a 'Normal' Ratio to be ~1.5. 
    # If Ratio is 3.0, stocks are 2x overvalued vs Gold.
    ratio_stress = (ratio - 1.5) / 1.5
    
    # Combined Stress Factor (Heavily weighted on Z-score)
    total_stress = (0.7 * z_score) + (0.3 * ratio_stress)
    
    # 2. CONTINUOUS EQUITY WEIGHT
    # As total stress goes from -1 to 3, Equity slides from 80% to 10%
    equity_weight = 10 + (70 / (1 + np.exp(2.0 * (total_stress - 1.2))))
    
    # 3. GOLD VS CASH TILT
    # If Yield Spread is healthy, we prefer Stocks. 
    # If Ratio is high (>2.0), we prefer Cash over Gold (Gold is also 'expensive' vs Stocks)
    # If Ratio is low (<1.0), we prefer Gold over Cash.
    gold_preference = 1 / (1 + np.exp(3.0 * (ratio - 1.2)))
    
    # Gold takes a larger share of the "Safety" bucket if Ratio is low
    safety_bucket = 100 - equity_weight
    gold_weight = safety_bucket * (0.3 + 0.4 * gold_preference)
    
    # Yield Curve Penalty (Applied to Equities)
    if yield_spread < 0:
        protection = equity_weight * 0.3
        equity_weight -= protection
        gold_weight += protection # Move to Gold during inversions
        
    cash_weight = 100 - equity_weight - gold_weight
    return round(equity_weight, 1), round(gold_weight, 1), round(cash_weight, 1)
...
def run_macro_analysis():
    ...
    calloc = calculate_continuous_alloc_v2(z_score, yield_spread, ratio)
    # --- 5. FORMAT OUTPUT ---
    report = (
        f"\n{'='*55}\n"
        ...
        f"-------------------------------------------------------\n"
        f" RECOMMENDED ALLOCATION:\n"
        f"   > Equities (VTI/SPY):  {calloc[0]}%\n"
        f"   > Gold (Physical/GLD): {calloc[1]}%\n"
        f"   > Cash (T-Bills/USD):  {calloc[2]}%\n"
